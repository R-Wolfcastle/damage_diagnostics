...
def prep(bike_file, out_ncf, out_hdf, level=3, plot=False, mucoef_region_type=1, up_factor=1, sliding_law=2):
...
#        c_rc[ocean]=0
        ...
        mucoef = {
            ...
            '2': lambda mucoef: np.maximum(0.9, mucoef),
            '3': lambda mucoef: np.where(btrac==0, np.maximum(0.9, mucoef), mucoef),
            '4': lambda mucoef: np.where(btrac==0, mucoef, np.maximum(0.9, mucoef))
        ...
        save_nc(x, y, {basal_friction_param_name: c_rc,  'mucoef': mucoef}, out_ncf)
        nctoamr(out_ncf, out_hdf, '{} mucoef'.format(basal_friction_param_name))

data_dir = "/nfs/b0133/eetss/BISICLES/diagnostic_damage_study/data/AIS/forward/2km_no_dreg/"
...
prep(ctrl_file, data_dir+"AIS_thermo_c_reg_clm_heal_shelves_01_thres.nc",
				data_dir+"AIS_thermo_c_reg_clm_heal_shelves_01_thres.hdf5",
				level=2, plot=False, mucoef_region_type=3,
				up_factor=1, sliding_law=1)
